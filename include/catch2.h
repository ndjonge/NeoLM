...
#define CATCH_INTERNAL_SUPPRESS_UNUSED_TEMPLATE_WARNINGS                                                               \
	_Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wunused-template\"")
#define CATCH_INTERNAL_UNSUPPRESS_UNUSED_TEMPLATE_WARNINGS _Pragma("clang diagnostic pop")
#endif // __clang__
	...
	{                                                                                                                  \
		...
		{                                                                                                              \
			...
			{                                                                                                          \
				...
				(void)expander{ (                                                                                      \
					reg_test(                                                                                          \
						Types{}, #ClassName, Catch::NameAndTags{ Name " - " + std::string(tmpl_types[index]), Tags }), \
					index++,                                                                                           \
					0)... }; /* NOLINT */                                                                              \
			}                                                                                                          \
		};                                                                                                             \
...
namespace Detail
{
inline std::string stringify(NSString* nsstring) { return StringMaker<NSString*>::convert(nsstring); }
...
template <typename T> void handleExpression(ExprLhs<T> const& expr) { handleExpression(expr.makeUnaryExpr()); }
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                                         \
	...
	{                                                                                                                  \
		...
		INTERNAL_CATCH_REACT(catchAssertionHandler)                                                                    \
			 ...
			 (false) && static_cast<bool>(!!(__VA_ARGS__))) // the expression here is never evaluated at runtime but it
...
template <typename T> MatchNotOf<T> MatcherBase<T>::operator!() const { return MatchNotOf<T>(*this); }
...
inline Impl::NSStringMatchers::Contains Contains(NSString* substr) { return Impl::NSStringMatchers::Contains(substr); }
...
inline Impl::NSStringMatchers::EndsWith EndsWith(NSString* substr) { return Impl::NSStringMatchers::EndsWith(substr); }
...
template <typename Clock> EnvironmentEstimate<FloatDuration<Clock>> estimate_clock_cost(FloatDuration<Clock> resolution)
{
	auto time_limit = std::min(
		resolution * clock_cost_estimation_tick_limit, FloatDuration<Clock>(clock_cost_estimation_time_limit));
...
auto StringRef::operator!=(StringRef const& other) const noexcept -> bool { return !operator==(other); }
...
bool isThrowSafe(TestCase const& testCase, IConfig const& config) { return !testCase.throws() || config.allowThrows(); }
	...
	std::string const& className, std::string const& rootName, SectionNode const& sectionNode)
{
	...
	if (!sectionNode.assertions.empty() || !sectionNode.stdOut.empty() || !sectionNode.stdErr.empty())
	{
		...
		if (!sectionNode.stdOut.empty()) xml.scopedElement("system-out").writeText(trim(sectionNode.stdOut), false);
		if (!sectionNode.stdErr.empty()) xml.scopedElement("system-err").writeText(trim(sectionNode.stdErr), false);
	}
...
void XmlReporter::testCaseEnded(TestCaseStats const& testCaseStats)
{
	...
	if (!testCaseStats.stdOut.empty()) m_xml.scopedElement("StdOut").writeText(trim(testCaseStats.stdOut), false);
	if (!testCaseStats.stdErr.empty()) m_xml.scopedElement("StdErr").writeText(trim(testCaseStats.stdErr), false);
